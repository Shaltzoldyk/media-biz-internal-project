...
export type YTDiscoveryConfig = {
  ...
  minAvgViews?:  number
  maxPages?:     number // default 10, same as original script
}
...
export async function discoverYTLeads(
  ...
  const channelIds = await searchChannels(
    config.keyword,
    config.maxPages ?? 10,
    apiKey
  )

  // Same 250-channel cap as original script
  const channelsData = await getChannelDetailsBatch(
    channelIds.slice(0, 250),
    apiKey
  )
  ...
  for (const channel of channelsData) {
    try {
      ...
      if (subs < config.minSubs || subs > config.maxSubs) continue
      ...
      if (uploads30d < config.minUploads30d) continue

      const stats    = await getVideoStats(videoIds.slice(0, 10), apiKey)
      const views    = stats.map((v: any) =>
      ...
      if (config.minAvgViews && avgViews < config.minAvgViews) continue

      // Same scoring formula as original script
      ...
      results.push({
        ...
        lastUpload:  lastUpload
          ? lastUpload.toISOString().split("T")[0]
          : "",
        email,
        website,
        instagram,
      ...
      // Skip channels that error — same behaviour as original
    }
  }
...
async function searchChannels(
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    const res = await fetch(`${BASE_URL}/search?${params}`).then((r) =>
      r.json()
    )
...
async function getChannelDetailsBatch(
  ids: string[],
  apiKey: string
): Promise<any[]> {
  const result: any[] = []
  ...
  for (let i = 0; i < ids.length; i += 50) {
    ...
    const res = await fetch(`${BASE_URL}/channels?${params}`).then((r) =>
      r.json()
    )
...
async function getRecentVideos(
  ...
  const res = await fetch(`${BASE_URL}/playlistItems?${params}`).then((r) =>
    r.json()
  )
  return res.items ?? []
}
