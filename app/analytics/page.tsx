...
// Currency formatting is context-based in client components.
// This is a server page — format raw INR values directly.
    ...
    <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px,1fr))", gap: 10 }}>
          {[
            ...
            <div key={s.label} className="card stat">
              ...
              <div className="val" style={{ fontSize: "1.15rem" }}>{inr(s.val)}</div>
            </div>
          ))}
        </div>

        {/* Trend + confidence inline */}
      ...
      <div className="fade-up delay-2" style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
          {[
            ...
            <div key={s.label} className="card stat">
              <div className="label">{s.label}</div>
              <div className="val" style={{ fontSize: "1.15rem", color: s.color }}>{inr(s.val)}</div>
            </div>
          ))}
        </div>
        <div className="card stat" style={{ marginTop: 10 }}>
          <div className="label">Volatility</div>
          <div className="val" style={{ fontSize: "1rem" }}>{inr(simulation.volatility)}</div>
        </div>
      </div>
      ...
      <div className="fade-up delay-2" style={{ marginBottom: 28 }}>
        ...
        <div className="card stat">
          ...
          <div className="val">{inr(pipeline.totalExpectedValue)}</div>
        </div>
      </div>
            ...
            <tbody>
              {Object.entries(pipeline.stageProbabilities || {}).map(([stage, prob]) => (
                <tr key={stage}>
                  ...
                  <td>
                    <span className={probPill(prob as number)}>
      ...
      <div className="fade-up delay-4">
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          <table>
            ...
            <tbody>
              {Object.entries((aging.buckets || aging) as Record<string, number>).map(([bucket, count]) => (
                <tr key={bucket}>
        ...
        {(aging as any).structuralRiskScore > 0.35 && (
          <div style={{
            ...
            <span style={{ color: "var(--text-2)" }}>
              {((aging as any).structuralRiskScore * 100).toFixed(1)}% risk score
            </span>
          </div>
        )}
      </div>
