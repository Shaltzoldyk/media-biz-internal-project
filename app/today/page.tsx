...
export default async function TodayPage() {
  ...
  const [{ data: ld }, { data: cd }, { data: rd }] = await Promise.all([
  ...
  const scoreColor = health.score >= 75 ? "var(--green)"
    : health.score >= 50 ? "var(--amber)" : "var(--red)"
  ...
  return (
    <div>
      ...
      <div className="fade-up delay-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <Feed label="Stuck leads" empty="No stuck leads."
          items={stuck.map((l: any) => ({
            id: l.leadId, href: `/leads/${l.leadId}`, title: l.name,
            sub: `${l.daysInStage}d in ${l.stage}`, right: l.severity,
            dot: l.severity === "critical" ? "dot-red" : "dot-amber",
          }))} />

        <Feed label="Overdue follow-ups" empty="No overdue follow-ups."
          items={overdue.map((l: any) => ({
            id: l.leadId, href: `/leads/${l.leadId}`, title: l.name,
            sub: l.stage, right: `${l.overdueDays}d overdue`, dot: "dot-red",
          }))} />

        <div style={{ gridColumn: "1 / -1" }}>
          <Feed label="At-risk clients" empty="No at-risk clients."
            items={atRisk.map((c: any) => ({
              id: c.clientId, href: `/clients/${c.clientId}`, title: c.name,
              sub: `${c.daysOverdue}d overdue`, right: c.severity,
