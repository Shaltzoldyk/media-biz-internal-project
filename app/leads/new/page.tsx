...
export default function NewLeadPage() {
  ...
  const [uploadsWeekly, setUploads]      = useState(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const score = calculateLeadScore({ subscribers: subs, outsourcing, uploadsWeekly, monetized, warmIntro })
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ fontSize: "0.78rem", fontWeight: 500, color: "var(--text-2)", marginBottom: 2 }}>
                  Scoring signals
                </div>
                ...
                <CheckField
                  ...
                  checked={warmIntro}
                  onChange={setWarmIntro}
                />
                <CheckField
                  label="Already outsourcing production"
                  checked={outsourcing}
                  onChange={setOutsourcing}
                />
                <CheckField
                  label="Uploads weekly or more"
                  checked={uploadsWeekly}
                  onChange={setUploads}
                />
                <CheckField
                  label="Monetized channel"
                  checked={monetized}
                  onChange={setMonetized}
                />
              </div>
